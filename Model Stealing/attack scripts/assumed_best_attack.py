...
# ======================= Student Model =======================
class StolenResNet34(nn.Module):
    def __init__(self):
        super().__init__()
...
def main():
    ...
    model = StolenResNet34().to(device)
    ...
    for epoch in range(80):
        ...
        for orig, teach, pils in loader:
            ...
            out1 = model(orig)
            out2 = model(aug)
            loss = F.mse_loss(out1, teach) + lambda_cos * cosine_loss(out1, teach)
            loss += lambda_aug * (F.mse_loss(out2, teach) + lambda_cos * cosine_loss(out2, teach))
            optimizer.zero_grad(); loss.backward(); optimizer.step()
            total_loss += loss.item() * orig.size(0)
        scheduler.step()
        print(f"Epoch {epoch+1}/80: Loss={total_loss/len(dataset):.4f}")
